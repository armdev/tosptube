...
public class AdvancedSearch implements Serializable {
    ...
    public AdvancedSearch() {
        ///System.out.println("AdvancedSearch called");
    }

    @SuppressWarnings("unchecked")
    public void doSearch() {
        if (searchQuery != null) {          
            Try<List> fetch = this.fetchList(searchQuery);
            if (fetch.isSuccess()) {
                videoList = fetch.get();
            }
        }
    ...
    public Try<List> fetchList(String query) {
        return Try.of(() -> restClient.getFullTextSearchResult(query));
    }
